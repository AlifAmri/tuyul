...
export default function PumpScoresPage() {
  ...
  // Fetch only pump scores data
  const { data: pumpScores, isLoading } = useQuery({
  ...
  const currentData = useMemo(() => {
    return mergeAndSortMarketData(pumpScores?.scores);
  }, [pumpScores?.scores, mergeAndSortMarketData]);
